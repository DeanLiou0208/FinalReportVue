<template>
  <RouterLink to="/memberInformation/shoppingcart"><p type="button">購物車</p></RouterLink>
      ...
      <tr>
        <th>
          ...
          <i
            class="bi"
            :class="{
              'bi-chevron-expand': !(datas.sort === 'name'),
              'bi-sort-down': datas.order === 'desc' && datas.sort === 'name',
              'bi-sort-up': datas.order === 'asc' && datas.sort === 'name',
            }"
        ...
        <th>
          ...
          <i
            class="bi"
            :class="{
              'bi-chevron-expand': !(datas.sort === 'price'),
              'bi-sort-up': datas.order === 'asc' && datas.sort === 'price',
              'bi-sort-down': datas.order === 'desc' && datas.sort === 'price',
            }"
            @click="sortHandler('price')"
          ></i>
        </th>
        <th>
          商品評價
          <i
            class="bi"
            :class="{
              'bi-chevron-expand': !(datas.sort === 'avgRateScore'),
              'bi-sort-up':
                datas.order === 'asc' && datas.sort === 'avgRateScore',
              'bi-sort-down':
                datas.order === 'desc' && datas.sort === 'avgRateScore',
            }"
            @click="sortHandler('avgRateScore')"
          ></i>
        </th>
        <th>
          商家名稱
          <i
            class="bi"
            :class="{
              'bi-chevron-expand': !(datas.sort === 'companyShopName'),
              'bi-sort-up':
                datas.order === 'asc' && datas.sort === 'companyShopName',
              'bi-sort-down':
                datas.order === 'desc' && datas.sort === 'companyShopName',
            }"
            @click="sortHandler('companyShopName')"
          ></i>
        </th>
      </tr>
    </thead>
  ...
  <div class="accordion accordion-flush" id="accordionFlushExample">
    ...
    <div class="accordion-item">
      ...
      >
        ...
        >
          您要的商品，在這裡點我「應該」都 &nbsp
          <i class="bi bi-search"></i> &nbsp 的到：
        </button>
      </h2>
      <div
  ...
  <div class="container ml-1">
    <div class="row">
      <div
        ...
        v-for="{ img, price, name, description, productId } in products"
        :key="productId"
      >
          ...
          <div class="card-body">
            ...
            <div class="container ml-1">
              ...
              <a
                ...
                ><i class="bi bi-cart-plus-fill"> >加入購物車</i></a
              >
            </div>
          </div>
        </div>
  ...
  <div
    ...
    tabindex="-1"
    aria-labelledby="addToShoppingCartLabel"
    aria-hidden="true"
  >
              ...
              <div class="col-12">
                <div class="card-add container ml-2" style="width: 100%">
                  ...
                  <div class="card-body container ml-3">
                    <div class="container ml-3" style="display: flex">
                      <p>ItemNo:</p>
                      <p class="card-number">{{ productDetails.productId }}</p>
                    </div>
                    <div class="container ml-3" style="display: flex">
                    ...
                    <div class="container ml-3" style="display: flex">
                      <p>售價:</p>
                      <p class="card-price">{{ productDetails.price }}</p>
                      <p>元</p>
                    </div>
                    <div class="container ml-3" style="display: flex">
...
const addShoppingCart = reactive({
  fkMemberId: 0,
  fkProductId: 0,
  count: 0,
});
...
onUpdated(() => {
  ...
  const element = document.querySelectorAll(
    'a[data-bs-toggle="modal"][data-bs-target="#addToShoppingCart"]'
  );
  const elements = document.querySelectorAll(
    'a[data-bs-toggle="modal2"][data-bs-target="#toProduct"]'
  );
  ...
  if ($cookies.get("identity") != null) {
    if ($cookies.get("identity") === "會員") {
      if ($cookies.get("fkMemberId") != null) {
        ...
        addShoppingCart.fkMemberId = fkMemberId.value;
        addShoppingCart.fkProductId = pdetails.productId;
        addShoppingCart.count = counting.value;
        console.log(addShoppingCart);
        if (addShoppingCart.count == 0) {
          Swal.fire({
            icon: "warning",
            title: "請輸入購買數量!",
          });
        } else {
          const response = await axios.put(URLADDSHOPPINGCART, addShoppingCart);
